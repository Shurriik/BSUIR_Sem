...
// Аппаратное включение звука на заданной частоте
void SoundOn(int freq) {
...
// Аппаратное выключение звука
void SoundOff() {
    unsigned char spkr = inp(SPEAKER_PORT);
    outp(SPEAKER_PORT, spkr & 0xFC);
}

// Аппаратная мелодия будильника iPhone
void iPhoneAlarmMelody() {
    // Нота E (659 Hz)
    SoundOn(659);
    delay(200);
...
void AlarmTriggered() {
    ...
    // Воспроизводим мелодию с проверкой нажатия
    for (i = 0; i < 3 && !stop; i++) {
