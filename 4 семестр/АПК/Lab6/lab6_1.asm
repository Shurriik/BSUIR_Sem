...
SHOW_TIMER:
    inc  ds:[TICKS]                             ;IRQ0 приходит примерно 18.2 раза в секунду
    cmp  ds:[TICKS],18
    jb   SKIP_SECOND
    mov  ds:[TICKS],0
...
DATA_BEGIN      = $
    ...
    MSG_BOX_TOP         db "+----------------------------------------------------------+",0
    MSG_HELLO_PM        db "|        PROTECTED MODE: IRQ0 TIMER + IRQ1 KEYBOARD        |",0
    MSG_BOX_DIV         db "|----------------------------------------------------------|",0
    MSG_KEYBOARD        db "|  Last keyboard scan-code:      h                         |",0
    MSG_TIME            db "|  Seconds before return:        s                         |",0
    MSG_COUNT           db "|  Timer IRQ0 calls:                                       |",0
    MSG_CR0             db "|  CR0:                                                    |",0
    MSG_HELP            db "|  Exit: press ESC or wait until timer reaches zero.       |",0
    MSG_BOX_BOTTOM      db "+----------------------------------------------------------+",0
    MSG_EXIT            db "Returned to real mode. Program finished safely.",13,10,"$"
    MSG_EXC             db "Exception: XX",0
    MSG_ENTER           db "Enter protected-mode time in seconds (1..255): $"
    MSG_ERROR           db "Incorrect input. Use a number from 1 to 255.$"
    ...
    TICKS               db  1 dup(0)            ;Количество тиков IRQ0 для отсчета секунды
    ...
    BUFFER_TIME         db  8 dup(' ')          ;Буфер для вывода оставшегося время в защищенном режиме
                        db  1 dup(0)
    INPUT_TIME          db  6,0,6 dup(?)          ;Буфер для ввода время
